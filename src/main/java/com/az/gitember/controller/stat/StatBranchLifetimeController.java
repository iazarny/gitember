package com.az.gitember.controller.stat;
...
public class StatBranchLifetimeController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        Context.scmStatBranchLiveTimeProperty.addListener(
                (observable, oldValue, newValue) -> {
                    ...
                    series.setName("Average branch live time in hours");

                    newValue.forEach(
                            blt -> {
                                float avg = 0;
                                if (blt.getQty() != 0) {
                                    avg = (float) blt.getHrs() / blt.getQty();
                                }

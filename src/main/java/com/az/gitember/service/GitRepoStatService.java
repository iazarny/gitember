...
public class GitRepoStatService {
     ...
     * using {@code project}'s cached {@link GitRepoService} (not closed here -- it is owned and
     * cached by {@code project}).
     */
    public WorkingCopyStat computeStats(Project project) throws Exception {
        return computeStats(project.getGitRepoService(), project.getProjectHomeFolder());
    }

    /**
     * Reads branch, ahead/behind, working-copy changes and last-fetch time for one repository,
     * opening (and closing) a transient {@link GitRepoService} for it.
     * @return RepoStats
     * */
    public WorkingCopyStat computeStats(String projectHome) throws Exception {
        try (GitRepoService svc = GitRepoService.of(projectHome)){
            return computeStats(svc, projectHome);
        }
    }

    private WorkingCopyStat computeStats(GitRepoService svc, String projectHome) throws Exception {
        String branch = "";
        int ahead = 0;
        int behind = 0;
